...
void MoreTeapotsModel::Init(const int32_t numX, const int32_t numY,
    ...
    float gap_x = teapot_x_==1?0:total_width / (teapot_x_ - 1);
    float gap_y = teapot_y_==1?0:total_width / (teapot_y_ - 1);
    float gap_z = teapot_z_==1?0:total_width / (teapot_z_ - 1);
    ...
    mat_view_ = Matrix4::makeTranslation(Cvec3(0,0,400.0f));
...
void MoreTeapotsModel::UpdateViewport() {
    ...
    mat_projection_ = Matrix4::makeProjection(40, aspect, CAM_NEAR, CAM_FAR);
}
...
void MoreTeapotsModel::Render(float r, float g, float b) {
    ...
    TEAPOT_MATERIALS material = {{1.0f, 1.0f, 1.0f, 40.f}, {0.1f, 0.1f, 0.1f}, };
    ...
    glUniform3f(moreTeapotsShaderState_->light0_, 0.f, 100.f, -300.f);
