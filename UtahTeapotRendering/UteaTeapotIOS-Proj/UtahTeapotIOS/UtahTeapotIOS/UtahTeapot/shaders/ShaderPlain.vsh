...
void main(void)
{
    ...
    //vec3 ecPosition = vec3(mat3(uMVMatrix[0].xyz, uMVMatrix[1].xyz, uMVMatrix[2].xyz) * myVertex);
    
    //colorDiffuse的计算有问题，不能向紫铜色靠拢，颜色的调试可以直接依靠fragment的输出
    colorDiffuse = max(0.0,dot(normalize(worldNormal), normalize(-vLight0 + ecPosition) )) * vMaterialDiffuse + vec4(vMaterialAmbient,1);
